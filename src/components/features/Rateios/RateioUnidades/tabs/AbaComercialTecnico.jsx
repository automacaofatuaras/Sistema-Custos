...
export default function AbaComercialTecnico({ selectedSegment, activeRateioType, calculatedData }) {
    if (selectedSegment === 'Concreteiras e Fábrica de Tubos') {
        ...
        const ccLabel = activeRateioType === 'COMERCIAL' ? '1105' : '1075';
        const colorTheme = activeRateioType === 'COMERCIAL' ? 'rose' : 'cyan';

        // Agrupamento para subtotais por CC (seguindo padrão das Pedreiras)
        ...
        return (
            <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border-l-4 border-indigo-500 shadow-sm"><p className="text-xs font-bold text-slate-500 uppercase">Produção Total (11 Unidades)</p><h3 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{totalProduction.toLocaleString()} <span className="text-sm font-normal text-slate-400">m³</span></h3></div>
                        ...
                        <table className="w-full text-sm text-left"><thead className="bg-slate-50 dark:bg-slate-900/50 text-slate-500 text-xs uppercase"><tr><th className="p-3 pl-6">Unidade</th><th className="p-3 text-right">Produção</th><th className="p-3 text-right">% Participação</th><th className="p-3 text-right">Valor do Rateio</th></tr></thead><tbody className="divide-y dark:divide-slate-700">
                            ...
                            {units.length > 0 && (
                                <tr className="bg-slate-50 dark:bg-slate-900/50 font-bold border-t-2 border-slate-200 dark:border-slate-700">
                                    <td className="p-3 pl-6 text-slate-700 dark:text-slate-200 uppercase text-xs">Total Geral</td>
                                    <td className="p-3 text-right font-mono text-slate-700 dark:text-slate-200">{totalProduction.toLocaleString()}</td>
                                    <td className="p-3 text-right text-slate-700 dark:text-slate-200">100.00%</td>
                                    <td className={`p-3 text-right text-${colorTheme}-700 dark:text-${colorTheme}-400`}>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-4">
                    <h5 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2 mb-3"><FileText size={16} /> Detalhamento das Despesas Rateadas (CC {ccLabel})</h5>
                    <div className="max-h-80 overflow-y-auto pr-2">
                        <table className="w-full text-xs text-left">
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                {Object.entries(groupedExpenses).map(([cc, data]) => (
