...
import { LayoutDashboard, FileText, BarChart2, Settings, UploadCloud } from 'lucide-react';
...
export default function RateioAdmCentral({ filter, setFilter, years }) {
    const [activeTab, setActiveTab] = useState('dashboard');

    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-6">Rateio Administrativo Geral</h2>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border dark:border-slate-700 p-6 min-h-[600px]">
                {activeTab === 'dashboard' && <DashboardGeral filter={filter} />}
                {activeTab === 'resumo' && <ResumoSegmento filter={filter} />}
                {activeTab === 'analise' && <AnaliseDetalhada filter={filter} />}
